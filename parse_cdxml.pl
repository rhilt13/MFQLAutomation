...
##	Fixed dbr formulae
##		Added nitrogen
##		Calculate both first and second value
##	Fixed order of first and second molecule
...
## To get help:
## perl parse_cdxml.pl --help
## Without fragmentation info:
## perl parse_cdxml.pl --in Test1.cdxml --out Test1OK --name NAME --code CODE --dbr auto --nO 0 --nC 0 --mode neg --OMe no
## With fragmentation info added:
## perl parse_cdxml.pl --in Test1.cdxml --out Test1OK --name NAME --code CODE --dbr auto --nO 0 --nC 0 --mode neg --OMe no --frag Frag1.cdxml --fragcode CODE1 --fragdbr auto --fragOMe No
## With multiple fragmentation info added:
## perl parse_cdxml.pl --in Test1.cdxml --out Test1OK --name NAME --code CODE --dbr auto --nO 0 --nC 0 --mode neg --OMe no --frag Frag1.cdxml,Frag2.cdxml,Frag3.cdxml --fragcode CODE1,CODE2,CODE3 --fragdbr auto --fragOMe No,No,No
...
if ($frag){
	...
	if($#fr!=$#frcode){die "ERROR: --frag and --fragcode entries counts do not match"};
	if($#fr!=$#frdbr){die "ERROR: --frag and --fragdbr entries counts do not match"};
	if($#fr!=$#frOMe){die "ERROR: --frag and --fragOMe entries counts do not match"};
...
sub GetValues {
	my ($name,$code,$d1,$d2,%val1,$chrg1,$ch,%val2,$valC,$valO,$check,@form,$c,%temp,$d_temp);
	my ($in, $dbr, $OMe) = @_;
	open(IN,$in);
	while(<IN>){
		chomp;
	...
	if ($dbr eq 'auto'){
		...
		if($dbr=~/^[\d.]+\-[\d.]+$/){
			...
			die "ERROR: Please enter dbr in format --dbr 1-2";
		}	
	}	
	my $i=0;
	foreach $comp(@form){
		...
		@d=split(/\//,$comp);
		if ($d[-1]!~/[-+]/){
			die "ERROR: Charge information not found in $in file. Please check the line with \">Chemical Formula\" in cdxml file.";
		}else{
			$chrg1=$d[-1];
		}
		$ch=substr($chrg1,0,1);
	...
	if ($dbr eq 'auto'){
		if (defined ($val1{'N'})){
			$d1=$val1{'C'}-$val1{'H'}/2 +$val1{'N'}/2+1;
			$d2=$val2{'C'}-$val2{'H'}/2 +$val2{'N'}/2+1;
		}else{
			$d1=$val1{'C'}-$val1{'H'}/2 +1;
			$d2=$val2{'C'}-$val2{'H'}/2 +1;
		}
	}
	if ($val1{'C'} > $val2{'C'}){
		%temp=%val1;
...
sub GetDefine {
	...
	if ($mode eq 'neg'){
		...
		die "ERROR: Please enter either pos2H, posNa or neg for option --mode\nDefault is neg\nUse perl $0 --help for more details.\n";
	}
	if (defined ($val1{'N'})){
		if ($check==1){
			$def.=" N[".$val1{'N'}."..$val2{'N'}"."]";
	...
	if (defined ($val1{'Cl'})){
		if ($check==1){
			$def.=" Cl[".$val1{'Cl'}."..$val2{'Cl'}"."]";
		}else{
			$def.=" Cl[".$val1{'Cl'}."..$val1{'Cl'}"."]";
		}
	}
	if (defined ($val1{'Br'})){
		if ($check==1){
			$def.=" Br[".$val1{'Br'}."..$val2{'Br'}"."]";
		}else{
			$def.=" Br[".$val1{'Br'}."..$val1{'Br'}"."]";
		}
	}
